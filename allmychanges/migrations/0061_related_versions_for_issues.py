...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for issue in orm['allmychanges.Issue'].objects.filter(type='lesser-version-count'):
            match = re.match(r"This time we didn't discover (.*) versions", issue.comment)
            if match is not None:
                issue.related_versions = match.group(1)
                issue.save()

        for issue in orm['allmychanges.Issue'].objects.filter(type='too-many-new-versions'):
            match = re.match(r"I found (.*)", issue.comment)
            if match is not None:
                issue.related_versions = match.group(1)
                issue.save()
