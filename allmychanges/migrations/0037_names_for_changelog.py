...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for changelog in orm['allmychanges.Changelog'].objects.all():
            preferences = []
            for package in changelog.packages.all():
                if package.user.username == 'svetlyak40wt':
                    preferences.append((package.namespace, package.name))
            for package in changelog.packages.all():
                if package.user.username == 'bessarabov':
                    preferences.append((package.namespace, package.name))
            for package in changelog.packages.all():
                if package.user.username not in ('svetlyak40wt', 'bessarabov'):
                    preferences.append((package.namespace, package.name))

            if preferences:
                changelog.namespace, changelog.name = preferences[0]
                changelog.save(update_fields=('namespace', 'name'))
